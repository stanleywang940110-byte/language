...
function render() {
  ...
  if (state.screen === 'start') {
    ...
    if (state.score === questions.length) {
      message = '恭喜全部答對！叫你第一名！';
      iconHTML = '<span class="material-icons result-icon perfect">emoji_events</span>';
    } else if (state.score >= 3) {
      message = '表現袂䆀！你有通過證照的潛力';
      iconHTML = '<span class="material-icons result-icon good">thumb_up</span>';
    } else {
      message = '拍斷手骨顛倒勇！台語有很多細節繼續努力';
      iconHTML = '<span class="material-icons result-icon keep-going">fitness_center</span>';
    }
        ...
        <div class="result-card">
          <div class="score-circle">
            ...
            <div class="score-text">
               <div class="score-number">${state.score}/${questions.length}</div>
               <div class="score-label">總得分</div>
            </div>
          </div>
          
          <div class="tutor-feedback-box">
             <img src="tutor_avatar.png" alt="Tutor" class="feedback-avatar">
             <div class="feedback-bubble">
                <div class="feedback-tag"><span class="material-icons" style="font-size: 1rem;">verified</span> 林老師講評</div>
                <div class="feedback-msg">${message}</div>
             </div>
          </div>
        </div>

        <button class="retry-btn" id="restart-btn">
          <span class="material-icons">replay</span> 重新測驗
        </button>
      </div>
    `;
